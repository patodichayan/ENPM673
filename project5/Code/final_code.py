...
def RANSAC_fundamental(X1, X2):
	...
	for i in range(M):
		...
		err = np.sum(np.dot(X2_3d, F_i)*X1_3d, axis=1)
...
def main():
	...
	for i in range(19, 3872):
		...
		T_t_cv = T_t_cv + np.dot(R_t_cv, t_cv)
		R_t_cv = np.dot(R_t_cv, R_cv)

		# drift calculation
		# uncomment to see drift values
		total_drift = trajectory_drift([T_t[0], T_t[2]], [T_t_cv[0], T_t_cv[2]], total_drift)
