    ...
    def run(self):
       ...
       ####1D-3Gaussian start##################################################################################################################################
        for iter in range(self.iterations):
            ...
            """
            for c,g,p in zip(range(3),[self.Gaussian1D(X_stack,self.mu[0],self.sigma[0]),
                                       self.Gaussian1D(X_stack,self.mu[1],self.sigma[1]),
                                       self.Gaussian1D(X_stack,self.mu[2],self.sigma[2])],self.pi):
            ...
            for g,c in zip([self.Gaussian1D((np.linspace(-25, 350, num=3900)),self.mu[0],self.sigma[0]),
                            self.Gaussian1D((np.linspace(-25, 350, num=3900)),self.mu[1],self.sigma[1]),
                            self.Gaussian1D((np.linspace(-25, 350, num=3900)),self.mu[2],self.sigma[2])],['r','g','b']):
                g_sum = g_sum + g
            if iter==9:
                ...
                plt.xlim(10,300)
                plt.title("Green Channel Green Buoy_1D-3Gaussian")
